...
export interface PluginSecretStoreLike {
...
export function createPluginContext(input: {
  ...
  secretStore: PluginSecretStoreLike;
  runtimeSubmit?: (input: unknown) => Promise<unknown>;
  eventSubscribe?: (listener: (event: import('@openagent/runtime').RuntimeUiEvent) => void) => () => void;
