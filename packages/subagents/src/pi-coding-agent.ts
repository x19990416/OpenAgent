...
import { resolvePiSessionFile } from '@openagent/pi-adapter';
import type { AgentRuntimeAdapter, AgentRuntimeRunInput, PlanExecutionContext, RuntimeTool, RuntimeToolExecutionContext, RuntimeToolExecutionInput } from '@openagent/runtime';
import type { PiCodingAgentTask, SubagentRunResult } from './subagent-types.js';
import { getOpenAgentPath } from '@openagent/runtime';
import { sanitizeModelReplayText } from '@openagent/pi-adapter';
...
export interface PiCodingAgentOptions {
  createRuntimeAdapter: () => AgentRuntimeAdapter;
}
...
export class PiCodingAgent {
  ...
  constructor(options: PiCodingAgentOptions) {
    this.adapter = options.createRuntimeAdapter();
  }
