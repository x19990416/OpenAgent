...
import type {
  ...
  WorkspaceMeta
} from '@openagent/shared-types/index';
...
function SkillPanel() {
  ...
  const stats = useMemo(() => {
    const stats = { total: skills.length, enabled: 0, plugin: 0, risky: 0 };
    for (const skill of skills) {
      if (skill.enabled) stats.enabled += 1;
      if (skill.source === 'plugin') stats.plugin += 1;
      if (skill.risk && skill.risk !== 'read') stats.risky += 1;
    }
    return stats;
  }, [skills]);
...
function PluginPanel() {
  ...
  const pluginStats = useMemo(() => {
    const stats = { total: registry.plugins.length, installed: 0, enabled: 0, loaded: 0, errors: 0 };
    for (const plugin of registry.plugins) {
      if (plugin.installed) stats.installed += 1;
      if (plugin.enabled) stats.enabled += 1;
      if (plugin.status === 'loaded') stats.loaded += 1;
      if (plugin.status === 'error') stats.errors += 1;
    }
    return stats;
  }, [registry.plugins]);
