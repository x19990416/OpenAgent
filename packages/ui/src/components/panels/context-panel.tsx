...
import type { RuntimeActivityItem, ToolCallItem, WorkspaceMeta } from '@openagent/shared-types/events';
...
export function ContextPanel({ workspace, tools, runtimeActivities = [], activeThreadId, onCompactThread }: ContextPanelProps) {
  ...
  const recentSkillActivities = runtimeActivities.filter((activity) => activity.kind === 'skill').slice(-6);
  ...
  return (
    <div className="section-list">
      ...
      <div className="section-card">
        ...
        {recentSkillActivities.length === 0 ? (
          <div className="body-copy-soft mt-8">暂无 Skill 记录。</div>
        ) : (
          <div className="section-list">
            {recentSkillActivities.map((activity, index) => (
