...
import type { ApprovalRequest, PlanStepItem, RunStatus, RuntimeActivityItem } from '@openagent/shared-types/events';
...
function summarizeRuntimeActivities(activities: RuntimeActivityItem[]) {
  ...
  for (const activity of activities) {
    ...
    if (activity.kind === 'read' || activity.kind === 'list') readCount += 1;
    if (activity.kind === 'write') writeCount += 1;
    if (activity.kind === 'search') searchCount += 1;
    if (activity.kind === 'command') commandCount += 1;
    if (activity.kind === 'tool') toolCount += 1;
    if (activity.kind === 'skill') skillCount += 1;
  }
  const parts: string[] = [];
