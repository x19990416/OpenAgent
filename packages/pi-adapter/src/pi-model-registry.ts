...
function getOpenAgentPath(...segments: string[]) {
  return path.join(process.env.OPENAGENT_HOME || path.join(os.homedir(), '.openagent'), ...segments);
}
...
export async function loginOpenAgentPiOAuthProvider(input: {
  ...
  await authStorage.login(input.providerId as Parameters<typeof authStorage.login>[0], {
    ...
    onDeviceCode: (info) => {
      const instructions = [
        `设备码：${info.userCode}`,
        `验证地址：${info.verificationUri}`,
        info.expiresInSeconds ? `过期时间：${info.expiresInSeconds} 秒` : undefined
      ].filter(Boolean).join('\n');
      void input.onAuth({ url: info.verificationUri, instructions });
    },
    ...
    .map(([providerId, models]) => {
      ...
      return {
        ...
        models: sortedModels.map((model) => ({
          ...
          capabilities: [
            ...(Array.isArray(model.input) ? model.input.map((value: unknown) => ({ value: String(value), label: String(value) })) : []),
