...
import type { PlanExecutionContext, RuntimeLogEntry, RuntimeTool, RuntimeToolExecutionResult } from '@openagent/runtime';
import { normalizePiToolParametersWithReport } from './pi-tool-schema.js';

export interface PiToolExecutor {
  execute(input: { toolName: string; toolCallId: string; args: unknown; signal: AbortSignal }): Promise<RuntimeToolExecutionResult>;
}
...
export function toPiToolDefinitions(tools: RuntimeTool[], executor: PiToolExecutor, options: { onLog?: (entry: RuntimeLogEntry) => void; getPlanContext?: () => PlanExecutionContext | null } = {}): ToolDefinition<any, unknown>[] {
    ...
    async execute(toolCallId: string, params: unknown, signal?: AbortSignal, onUpdate?: (partialResult: { content: Array<{ type: 'text'; text: string }>; details: unknown }) => void) {
      ...
      if (jsonRetry) {
        ...
        options.onLog?.({
          ...
          data: { toolCallId, ...jsonRetry.details }
        });
        const retryResult = {
