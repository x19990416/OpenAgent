...
let loggerHooks: PiHttpLoggerHooks = {};

export function configurePiHttpLoggerHooks(hooks: PiHttpLoggerHooks) {
  loggerHooks = hooks;
}

function appendLlmResponseLog(input: { scope: string; message: string; data?: unknown }) {
  loggerHooks.appendLlmResponseLog?.(input);
}

function recordModelUsageFromResponse(input: Parameters<NonNullable<PiHttpLoggerHooks['recordModelUsageFromResponse']>>[0]) {
  return loggerHooks.recordModelUsageFromResponse?.(input) ?? null;
}
