...
import type { AgentRuntimeAdapter, AgentRuntimeCompactInput, AgentRuntimeCompactResult, AgentRuntimeRunInput, AgentRuntimeRunResult, RuntimeMessage } from '@openagent/runtime';
import { errorToMessage, isCancelledError } from '@openagent/runtime';
import { createPiModelContext, resolvePiThinkingLevel } from './pi-model-registry.js';
import { type PiToolExecutor, toPiToolDefinitions } from './pi-tools.js';
...
export interface PiRuntimeAdapterDependencies {
  createToolExecutor: (input: AgentRuntimeRunInput) => PiToolExecutor;
...
export class PiRuntimeAdapter implements AgentRuntimeAdapter {
  constructor(private readonly dependencies: PiRuntimeAdapterDependencies) {}

  private allowTextToolCallRecovery() {
    const envOverride = allowTextToolCallRecoveryFromEnv();
    if (typeof envOverride === 'boolean') return envOverride;
    return this.dependencies.allowTextToolCallRecovery?.() ?? true;
  }
  ...
  async run(input: AgentRuntimeRunInput): Promise<AgentRuntimeRunResult> {
    try {
      ...
      const toolExecutor = this.dependencies.createToolExecutor(input);
      ...
      this.dependencies.appendLlmResponseLog?.({
        scope: 'agent-loop',
      ...
      const toolLoopGuardSettings = this.dependencies.getToolLoopGuardSettings?.() ?? {};
      let promptResult = await promptOpenAgentPiSession({
        ...
        toolLoopGuard: {
          maxConsecutiveSameToolCalls: toolLoopGuardSettings.maxConsecutiveSameToolCalls,
          maxConsecutiveSameToolResults: toolLoopGuardSettings.maxConsecutiveSameToolResults
        }
      });
      ...
      for (let recoveryAttempt = 0; recoveryAttempt < 5; recoveryAttempt += 1) {
        ...
        const recoveryEnabled = this.allowTextToolCallRecovery();
        input.onLog?.({
          scope: 'agent-loop',
...
function allowTextToolCallRecoveryFromEnv() {
...
export namespace PiRuntimeAdapter {
  export type Dependencies = PiRuntimeAdapterDependencies;
}
