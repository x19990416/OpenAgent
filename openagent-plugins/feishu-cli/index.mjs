    ...
    register(ctx) {
        ...
        ctx.registerTool({
            ...
            execute: async ({ input, signal }) => {
                ...
                if (operation === 'test_connection') {
                    ...
                    if (!result.ok)
                        return { ok: false, content: result.message, data: result };
                ...
                if (operation === 'send_text_message') {
                    ...
                    if (!receiveId || !text)
                        return { ok: false, content: 'receiveId and text are required' };
                ...
                if (operation === 'reply_to_current_chat') {
                    ...
                    if (!chatId)
                        return { ok: false, content: 'chatId is required until Feishu channel metadata is implemented' };
        ...
        ctx.registerTool({
            ...
            execute: async ({ input, signal }) => {
                ...
                if (!receiveId || !text)
                    return { ok: false, content: 'receiveId and text are required' };
                const allowedChatIds = await ctx.config.get('allowedChatIds');
...
async function ensureCliConfigured(ctx, cliPath, signal) {
    ...
    const cli = await runCli(cliPath, ['config', 'init', '--name', FEISHU_PROFILE, '--app-id', appId, '--app-secret-stdin', '--brand', 'feishu'], signal, 30000, `${appSecret}\n`);
    if (cli.exitCode !== 0) {
...
function buildCalendarAgendaArgs(input) {
    const cliArgs = withProfile(['calendar', '+agenda', '--format', 'json', '--calendar-id', input.calendarId, '--as', input.identity]);
    if (input.start)
        cliArgs.push('--start', input.start);
    if (input.end)
        cliArgs.push('--end', input.end);
    return cliArgs;
}
async function listCalendars(cliPath, identity, signal) {
    return runCli(cliPath, withProfile(['calendar', 'calendars', 'list', '--format', 'json', '--as', identity, '--page-all']), signal);
}
async function readCalendarAgenda(cliPath, input, signal) {
    if (input.calendarId) {
...
function extractCalendars(input) {
    const seen = new Set();
    const calendars = [];
    const visit = (value) => {
        if (!value)
...
function looksLikeCalendar(record) {
...
function buildSendTextArgs(receiveIdType, receiveId, text) {
    const targetArgs = receiveIdType === 'chat_id'
        ? ['--chat-id', receiveId]
        : ['--user-id', receiveId];
    return withProfile(['im', '+messages-send', '--as', 'bot', ...targetArgs, '--text', text]);
}
function validateGenericCliArgs(args, expectedRisk) {
    if (args.length === 0)
        return { ok: false, content: 'cliArgs is required for operation=cli' };
    if (args.some((arg) => arg === '--profile'))
        return { ok: false, content: 'Do not pass --profile; feishu_agent injects --profile openagent.' };
    const blocked = new Set(['config', 'profile', 'update']);
    if (blocked.has(args[0]))
        return { ok: false, content: `lark-cli ${args[0]} is managed by OpenAgent plugin settings and cannot be run through feishu_agent.` };
...
async function runCli(command, args, signal, timeoutMs = 30000, stdin) {
...
function getPrivateLarkCliPath(pluginId) {
...
export default feishuCliPlugin;
