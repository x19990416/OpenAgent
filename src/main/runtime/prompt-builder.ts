...
export function buildSystemPrompt(input: Pick<PromptBuilderInput, 'workspaceRoot' | 'agentId'>) {
  return [
    ...
    '- Tool-call selection should use the current user prompt, current attachments, selected skill, active plan, and exposed tool schemas as the source of truth; do not copy old tool arguments from prior transcript unless the current prompt explicitly refers to them.',
    '',
