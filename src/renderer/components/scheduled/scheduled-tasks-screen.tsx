...
import type { CreateScheduledTaskInput, ScheduledTaskFrequency, ScheduledTaskItem, ScheduledTaskSessionPolicy } from '@shared-types/index';
...
function buildStats(tasks: ScheduledTaskItem[]) {
  ...
  const succeeded = tasks.filter((task) => task.lastRunStatus === 'succeeded').length;
  const nextRun = tasks
    .map((task) => (task.nextRunAt ? new Date(task.nextRunAt).getTime() : Number.POSITIVE_INFINITY))
  ...
  return {
    ...
    paused,
    succeeded,
    nextRunLabel: nextRun ? formatDateTime(new Date(nextRun).toISOString()) : '—'
  };
}
